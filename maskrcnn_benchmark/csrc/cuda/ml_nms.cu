...
__global__ void ml_nms_kernel(const int n_boxes, const float nms_overlap_thresh,
  ...
  if (threadIdx.x < row_size) {
    ...
    const int col_blocks = ((int)(n_boxes) + threadsPerBlock - 1);
...
at::Tensor ml_nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  const int col_blocks = ((int)(boxes_num) + threadsPerBlock - 1);
  ...
  dim3 blocks((int)(boxes_num + threadsPerBlock - 1),
              ((int)(boxes_num + threadsPerBlock - 1)));
